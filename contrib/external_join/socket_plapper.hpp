/*
 * @Author: your name
 * @Date: 2021-05-14 03:07:58
 * @LastEditTime: 2021-05-14 03:07:58
 * @LastEditors: your name
 * @Description: In User Settings Edit
 * @FilePath: /openGauss-server/contrib/GpuJoin/socker_plapper.hpp
 */
#ifndef SOCKETLAPPER_HEAD_
#define SOCKETLAPPER_HEAD_

static inline int setOptionSock(const int sock)
{
    int optval = 1;
...
static inline int listenSock(const int port)
{
    int sock;
...
static inline int acceptSock(const int server_sock)
{
    int client_sock;
    struct sockaddr_in addr;
...
static inline int connectSock(char* ip, const int port)
{
    int sock;
    struct sockaddr_in addr;
